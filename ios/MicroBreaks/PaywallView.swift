...
struct PaywallView: View {
    ...
    private let bullets = [
        ...
        "Randomized prompts so you don\u{2019}t clock-watch",
        "A timer for evidence-aligned rest habits \u{2014} not a medical device; results vary"
    ]
    ...
    private var card: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if founderUnlocked {
                Text("You're in \u{2014} early iOS access is unlocked.")
                    .font(MBType.label())
    ...
    private var priceLabel: String {
        ...
        if let product = store.product {
            return "Unlock early access \u{2014} \(product.displayPrice)"
        }
        return "Unlock early access \u{2014} $12"
    }
